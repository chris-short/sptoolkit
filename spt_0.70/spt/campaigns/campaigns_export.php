...
//formulate sql query
$sql = "SELECT campaigns.campaign_name AS campaign_name, templates.name AS template_name, education.name AS education_name, campaigns.education_timing AS education_timing, campaigns.date_sent AS date_sent, campaigns_responses.post AS post, targets.email AS email, targets.fname AS fname, targets.lname AS lname, campaigns_responses.ip AS ip, campaigns_responses.browser AS browser, campaigns_responses.browser_version AS browser_version, campaigns_responses.os AS os, campaigns_responses.link_time AS link_time, targets.group_name AS group_name";
...
//set header
$output = "Campaign,Template,Education,EducationTiming,Date,Year,Time,FirstName,LastName,TargetEmail,LinkTime,Post,IP,Browser,OS,Group";
...
while ( $ra = mysql_fetch_assoc ( $r ) ) {
    ...
    $output .= $ra['date_sent'] . "," . $ra['fname'] . "," . $ra['lname'] . "," . $ra['email'] . "," . $ra['link_time'] . "," . $ra['post'] . "," . $ra['ip'] . "," . $ra['browser'] . "," . $ra['os'] . "," . $ra['group_name'];
    //get all the custom metrics from the targets table and into an array
    $metrics = mysql_query ( "SHOW COLUMNS FROM targets" );
    while ( $metrics_results = mysql_fetch_array ( $metrics ) ) {
        if ( $metrics_results['Field'] != "id" && $metrics_results['Field'] != "email" && $metrics_results['Field'] != "fname" && $metrics_results['Field'] != "lname" && $metrics_results['Field'] != "group_name" ) {
